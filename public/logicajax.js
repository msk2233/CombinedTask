...
async function submitdata() {
  ...
  var form = document.getElementById('regForm');
    ...
    if (submittype == 'insert'){
      var url = "http://localhost:5000/post_data"
      ...
      var url = "http://localhost:5000/update_data"
      var res1 = await fetch(url, {
        method: "POST",
        body: formData,
        headers: {
...
async function state() {
  var url = await fetch('http://localhost:5000/get_state')
      .then(resp => resp.json())
...
async function selectcity(id) {
  var url = await fetch(`http://localhost:5000/get_city/?id=${id}`)
      .then(resp => resp.json())
