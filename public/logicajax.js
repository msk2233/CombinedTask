...
async function submitdata() {
  ...
  if (submittype == 'insert') {
    ...
    var url = `${location.origin}/dashboard/ajax_main/update`
    var res1 = await fetch(url, {
      method: "POST",
      body: formData,
      headers: {
...
async function state() {
  var url = await fetch(`${location.origin}/dashboard/ajax_main/get_state`)
  url = await url.json();
...
async function selectcity(id) {
  var url = await fetch(`${location.origin}/dashboard/ajax_main/get_city/?id=${id}`)
    .then(resp => resp.json())
